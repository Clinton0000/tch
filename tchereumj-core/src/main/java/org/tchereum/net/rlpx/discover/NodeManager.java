...
import org.tcheum.db.PeerSource;
...
public class NodeManager implements Consumer<DiscoveryEvent>{
    ...
    @Autowired
    public NodeManager(SystemProperties config, tcheumListener tcheumListener,
    ...
    public synchronized NodeHandler getNodeHandler(Node n) {
        ...
        if (ret == null) {
            ...
            if (!n.isDiscoveryNode() && !n.getHexId().equals(homeNode.getHexId())) {
                tcheumListener.onNodeDiscovered(ret.getNode());
            }
    ...
    /**
     * Checks whtch peers with such InetSocketAddress has penalize disconnect record
