 ...
 *
 * This sample connects to the test network (it has a lot of free tchs) and starts periodically
...
public class PendingStateSample extends TestNetSample {
    ...
    void onPendingTransactionReceived(Transaction tx) {
        ...
        if (Arrays.equals(tx.getSender(), senderAddress)) {
            BigInteger receiverBalance = tcheum.getRepository().getBalance(receiverAddress);
