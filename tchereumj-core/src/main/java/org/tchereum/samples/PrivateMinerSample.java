...
import org.tcheum.mine.EthashListener;
 ...
 * tcheumJ peers in a single JVM. For this two Spring ApplicationContext's are created which
...
public class PrivateMinerSample {
            ...
            for (int i = tcheum.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000; i++, j++) {
                {
                    Transaction tx = new Transaction(ByteUtil.intToBytesNoLeadZeroes(i),
                            ...
                            receiverAddr, new byte[]{77}, new byte[0], tcheum.getChainIdForNextBlock());
                    tx.sign(senderKey);
    ...
    /**
     *  Creating two tcheumJ instances with different config classes
     */
    public static void main(String[] args) throws Exception {
        ...
        tcheumFactory.createtcheum(MinerConfig.class);
