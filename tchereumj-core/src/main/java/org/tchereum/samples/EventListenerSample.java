...
import org.tcheum.db.TransactionStore;
import org.tcheum.facade.tcheumFactory;
import org.tcheum.listener.BlockReplay;
import org.tcheum.listener.tcheumListener;
import org.tcheum.listener.tcheumListenerAdapter;
import org.tcheum.listener.EventListener;
import org.tcheum.listener.TxStatus;
 ...
 *
 *  - getting free tch assuming we are running in test network
...
//            initContractTopic(contractABI, sha3("Inc(address,int256,int256)".getBytes()));
        ...
        @Override
        protected IncEvent onEvent(CallTransaction.Invocation event, Block block, TransactionReceipt receipt, int txCount, tcheumListener.PendingTransactionState state) {
    ...
    public void onSyncDone() throws Exception {
        ...
        requestFreetch(ECKey.fromPrivate(senderPrivateKey).getAddress());
        requestFreetch(ECKey.fromPrivate(sender2PrivateKey).getAddress());
    ...
    public void requestFreetch(byte[] addressBytes) {
        String address = "0x" + toHexString(addressBytes);
        logger.info("Checking address {} for available tch.", address);
        BigInteger balance = tcheum.getRepository().getBalance(addressBytes);
        logger.info("Address {} balance: {} wei", address, balance);
        BigInteger requiredBalance = BigInteger.valueOf(3_000_000 * tcheum.getGasPrice());
        if (balance.compareTo(requiredBalance) < 0) {
            logger.info("Insufficient funds for address {}, requesting free tch", address);
            try {
                String result = postQuery("https://ropsten.faucet.b9lab.com/tap", "{\"toWhom\":\"" + address + "\"}");
                logger.info("Answer from free tch API: {}", result);
                waitFortch(addressBytes, requiredBalance);
            } catch (Exception ex) {
                logger.error("Error during request of free tch,", ex);
            }
        }
    }
    ...
    private void waitFortch(byte[] address, BigInteger requiredBalance) throws InterruptedException {
        while(true) {
            BigInteger balance = tcheum.getRepository().getBalance(address);
            if (balance.compareTo(requiredBalance) > 0) {
    ...
    private void deployContractAndTest() throws Exception {
        ...
        tcheum.addListener(eventListener.listener);
        ...
        ProgramResult r = tcheum.callConstantFunction(Hex.toHexString(address),
    ...
    private void replayOnly() throws Exception {
        ...
        tcheum.addListener(blockReplay);
    ...
    protected TransactionReceipt sendTxAndWait(byte[] receiveAddress,
        ...
        BigInteger nonce = tcheum.getRepository().getNonce(ECKey.fromPrivate(privateKey).getAddress());
