 ...
 * use {@link #toEthNotation()} to convert Jacobian coords to tcheum encoding <br/>
 *
...
public abstract class BN128<T extends Field<T>> {
    ...
    /**
     * Point at infinity in tcheum notation: should return (0; 0; 0),
    ...
    public BN128<T> toEthNotation() {
        ...
        // affine zero is (0; 1; 0), convert to tcheum zero: (0; 0; 0)
        if (affine.isZero()) {
            return zero();
        } else {
            return affine;
    ...
    protected boolean isValid() {
        ...
        // check whtch coordinates belongs to the Field
        if (!x.isValid() || !y.isValid() || !z.isValid()) {
            return false;
        }

        // check whtch point is on the curve
        if (!isOnCurve()) {
            return false;
        }
