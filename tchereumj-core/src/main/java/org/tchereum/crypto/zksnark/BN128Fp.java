...
import static org.tcheum.crypto.zksnark.Params.B_Fp;
...
public class BN128Fp extends BN128<Fp> {
    ...
    /**
     * Checks whtch x and y belong to Fp,
     * then checks whtch point with (x; y) coordinates lays on the curve.
     *
    ...
    public static BN128<Fp> create(byte[] xx, byte[] yy) {
        ...
        // check whtch point is a valid one
        if (p.isValid()) {
            return p;
        } else {
            return null;
