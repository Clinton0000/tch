...
import org.tcheum.net.client.Capability;
import org.tcheum.net.eth.handler.Eth63;
import org.tcheum.net.message.ReasonCode;
import org.tcheum.net.server.Channel;
import org.tcheum.trie.TrieKey;
import org.tcheum.util.*;
...
import static org.tcheum.listener.tcheumListener.SyncState.COMPLETE;
import static org.tcheum.listener.tcheumListener.SyncState.SECURE;
import static org.tcheum.listener.tcheumListener.SyncState.UNSECURE;
import static org.tcheum.trie.TrieKey.fromPacked;
import static org.tcheum.util.CompactEncoder.hasTerminator;
...
public class FastSyncManager {
    ...
    @Autowired
    CompositetcheumListener listener;
    ...
    public SyncStatus getSyncState() {
        ...
        tcheumListener.SyncState syncStage = getSyncStage();
        switch (syncStage) {
            case UNSECURE:
...
//                idle.getEthHandler().setStatus(SyncState.IDLE);
    ...
    private void setSyncStage(tcheumListener.SyncState stage) {
        if (stage == null) {
    ...
    private tcheumListener.SyncState getSyncStage() {
        byte[] bytes = blockchainDB.get(FASTSYNC_DB_KEY_SYNC_STAGE);
        if (bytes == null) return UNSECURE;
        return tcheumListener.SyncState.values()[bytes[0]];
    }
    ...
    private void syncUnsecure(BlockHeader pivot) {
        ...
        listener.addListener(new tcheumListenerAdapter() {
            @Override
...
//        }
    ...
    private void syncSecure() {
        ...
        setSyncStage(tcheumListener.SyncState.SECURE);
    ...
    private void syncBlocksReceipts() {
        ...
        if (!config.fastSyncSkipHistory()) {
            ...
            setSyncStage(tcheumListener.SyncState.COMPLETE);
    ...
    public void main() {
        ...
        if (blockchain.getBestBlock().getNumber() == 0 || getSyncStage() == SECURE || getSyncStage() == COMPLETE) {
            ...
            try {
                tcheumListener.SyncState origSyncStage = getSyncStage();
                ...
                switch (origSyncStage) {
                    case UNSECURE:
                        ...
                        if (pivot.getNumber() == 0) {
                            ...
                            syncManager.initRegularSync(tcheumListener.SyncState.COMPLETE);
                            return;
                        }
                    ...
                    case SECURE:
                        if (origSyncStage == SECURE) {
                            ...
                            syncManager.initRegularSync(tcheumListener.SyncState.UNSECURE);
                        }

                        syncSecure();

                        listener.onSyncDone(tcheumListener.SyncState.SECURE);
                    case COMPLETE:
                        if (origSyncStage == COMPLETE) {
                            logger.info("FastSync: SECURE sync was completed prior to this run, proceeding with next stage...");
                            logger.info("Initializing regular sync");
                            syncManager.initRegularSync(tcheumListener.SyncState.SECURE);
                        }

                        syncBlocksReceipts();

                        listener.onSyncDone(tcheumListener.SyncState.COMPLETE);
                }
