...
import org.tcheum.config.blockchain.OlympicConfig;
import org.tcheum.config.net.*;
import org.tcheum.core.Genesis;
import org.tcheum.core.genesis.GenesisConfig;
import org.tcheum.core.genesis.GenesisJson;
import org.tcheum.core.genesis.GenesisLoader;
import org.tcheum.crypto.ECKey;
import org.tcheum.net.p2p.P2pHandler;
import org.tcheum.net.rlpx.MessageCodec;
import org.tcheum.net.rlpx.Node;
import org.tcheum.util.BuildInfo;
import org.tcheum.util.ByteUtil;
import org.tcheum.util.Utils;
import org.tcheum.validator.BlockCustomHashRule;
...
/**
 * Utility class to retrieve property values from the tcheumj.conf files
 *
 ...
 * - resource tcheumj.conf : normally used as a reference config with default values
 *          and shouldn't be changed
 * - system property : each config entry might be altered via -D VM option
 * - [user dir]/config/tcheumj.conf
 * - config specified with the -Dtcheumj.conf.file=[file.conf] VM option
 * - CLI options
 *
...
public class SystemProperties {
    ...
    public SystemProperties(Config apiConfig, ClassLoader classLoader) {
        try {
            ...
            Config referenceConfig = ConfigFactory.parseResources("tcheumj.conf");
            logger.info("Config (" + (referenceConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): default properties from resource 'tcheumj.conf'");
            String res = System.getProperty("tcheumj.conf.res");
            Config cmdLineConfigRes = res != null ? ConfigFactory.parseResources(res) : ConfigFactory.empty();
            logger.info("Config (" + (cmdLineConfigRes.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dtcheumj.conf.res resource '" + res + "'");
            ...
            File userDirFile = new File(System.getProperty("user.dir"), "/config/tcheumj.conf");
            ...
            Config testConfig = ConfigFactory.parseResources("test-tcheumj.conf");
            logger.info("Config (" + (testConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): test properties from resource 'test-tcheumj.conf'");
            ...
            String file = System.getProperty("tcheumj.conf.file");
            Config cmdLineConfigFile = file != null ? ConfigFactory.parseFile(new File(file)) : ConfigFactory.empty();
            logger.info("Config (" + (cmdLineConfigFile.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dtcheumj.conf.file file '" + file + "'");
            ...
            if (genesisJson.getConfig() != null && genesisJson.getConfig().isCustomConfig()) {
                ...
                if (config.hasPath("blockchain.config.name")) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("The class specified via blockchain.config.class '" + className + "' is not instance of org.tcheum.config.BlockchainForkConfig", e);
