...
import static org.tcheum.core.BlockchainImpl.calcReceiptsTrie;
...
public class BlockchainValidation {
    ...
    public static void checkNodes(tcheum tcheum, CommonConfig commonConfig, AtomicInteger fatalErrors) {
        try {
            Source<byte[], byte[]> stateDS = commonConfig.stateSource();
            byte[] stateRoot = tcheum.getBlockchain().getBestBlock().getHeader().getStateRoot();
    ...
    public static void checkHeaders(tcheum tcheum, AtomicInteger fatalErrors) {
        int blockNumber = (int) tcheum.getBlockchain().getBestBlock().getHeader().getNumber();
        byte[] lastParentHash = null;
        testLogger.info("Checking headers from best block: {}", blockNumber);

        try {
            while (blockNumber >= 0) {
                Block currentBlock = tcheum.getBlockchain().getBlockByNumber(blockNumber);
    ...
    public static void checkFastHeaders(tcheum tcheum, CommonConfig commonConfig, AtomicInteger fatalErrors) {
        ...
        try {
            ...
            Block genesis = tcheum.getBlockchain().getBlockByNumber(0);
    ...
    public static void checkBlocks(tcheum tcheum, AtomicInteger fatalErrors) {
        Block currentBlock = tcheum.getBlockchain().getBestBlock();
        int blockNumber = (int) currentBlock.getHeader().getNumber();

        try {
            BlockStore blockStore = tcheum.getBlockchain().getBlockStore();
            ...
            while (blockNumber > 0) {
                currentBlock = tcheum.getBlockchain().getBlockByNumber(blockNumber);

                // Validate uncles
                assert ((BlockchainImpl) tcheum.getBlockchain()).validateUncles(currentBlock);
            ...
            currentBlock = tcheum.getBlockchain().getBlockByNumber(0);
    ...
    public static void checkTransactions(tcheum tcheum, AtomicInteger fatalErrors) {
        int blockNumber = (int) tcheum.getBlockchain().getBestBlock().getHeader().getNumber();
        ...
        try {
            while (blockNumber > 0) {
                ...
                for (Transaction tx : currentBlock.getTransactionsList()) {
                    TransactionInfo txInfo = ((BlockchainImpl) tcheum.getBlockchain()).getTransactionInfo(tx.getHash());
                    assert txInfo != null;
    ...
    public static void fullCheck(tcheum tcheum, CommonConfig commonConfig, AtomicInteger fatalErrors) {

        // nodes
        testLogger.info("Validating nodes: Start");
        BlockchainValidation.checkNodes(tcheum, commonConfig, fatalErrors);
        testLogger.info("Validating nodes: End");

        // headers
        testLogger.info("Validating block headers: Start");
        BlockchainValidation.checkHeaders(tcheum, fatalErrors);
        testLogger.info("Validating block headers: End");

        // blocks
        testLogger.info("Validating blocks: Start");
        BlockchainValidation.checkBlocks(tcheum, fatalErrors);
        testLogger.info("Validating blocks: End");

        // receipts
        testLogger.info("Validating transaction receipts: Start");
        BlockchainValidation.checkTransactions(tcheum, fatalErrors);
