...
import static org.tcheum.util.ByteUtil.intToBytes;
import static org.tcheum.util.blockchain.tchUtil.Unit.tch;
import static org.tcheum.util.blockchain.tchUtil.convert;
...
//        System.out.println("Gen root: " + Hex.toHexString(bc.getBlockchain().getBestBlock().getStateRoot()));
        ...
        bc.sendtch(alice.getAddress(), convert(3, tch));
        ...
        bc.sendtch(bob.getAddress(), convert(5, tch));
        bc.createBlock();
        ...
        Assert.assertEquals(convert(30, tch), bc.getBlockchain().getRepository().getBalance(alice.getAddress()));
        Assert.assertEquals(convert(50, tch), bc.getBlockchain().getRepository().getBalance(bob.getAddress()));

        {
            ...
            Assert.assertEquals(convert(3 * 9, tch), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 9, tch), r1.getBalance(bob.getAddress()));
        }

        {
            Block b1 = bc.getBlockchain().getBlockByNumber(bestBlockNum - 2);
            Repository r1 = bc.getBlockchain().getRepository().getSnapshotTo(b1.getStateRoot());
            Assert.assertEquals(convert(3 * 8, tch), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 8, tch), r1.getBalance(bob.getAddress()));
        }

        {
            Block b1 = bc.getBlockchain().getBlockByNumber(bestBlockNum - 3);
            Repository r1 = bc.getBlockchain().getRepository().getSnapshotTo(b1.getStateRoot());
            Assert.assertEquals(convert(3 * 7, tch), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 7, tch), r1.getBalance(bob.getAddress()));
        }
...
//        repository.getDetailsDataStore().setStorageDS(storageDS);
    ...
    public void rewriteSameTrieNode() throws Exception {
        ...
        bc.sendtch(receiver, BigInteger.valueOf(0x77777777));
        bc.createBlock();

        for (int i = 0; i < 100; i++) {
            bc.sendtch(new ECKey().getAddress(), BigInteger.valueOf(i));
        }
