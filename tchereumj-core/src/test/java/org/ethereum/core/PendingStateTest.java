...
import static org.tcheum.listener.tcheumListener.PendingTransactionState.*;
...
public class PendingStateTest {
    ...
    static class PendingListener extends tcheumListenerAdapter {
...
//        public BlockingQueue<Triple<TransactionReceipt, PendingTransactionState, Block>> onPendingTransactionUpdate = new LinkedBlockingQueue<>();
    ...
    public void testSimple() throws InterruptedException {
        ...
        bc.addtcheumListener(l);
        Triple<TransactionReceipt, tcheumListener.PendingTransactionState, Block> txUpd;
        PendingStateImpl pendingState = (PendingStateImpl) bc.getBlockchain().getPendingState();

        ECKey alice = new ECKey();

        bc.sendtch(new byte[20], BigInteger.valueOf(100000));
        bc.sendtch(new byte[20], BigInteger.valueOf(100000));
    ...
    public void testRebranch1() throws InterruptedException {
        ...
        PendingListener l = new PendingListener();
        bc.addtcheumListener(l);
        Triple<TransactionReceipt, tcheumListener.PendingTransactionState, Block> txUpd = null;
        PendingStateImpl pendingState = (PendingStateImpl) bc.getBlockchain().getPendingState();
        ...
        bc.sendtch(bob.getAddress(), convert(100, tch));
        bc.sendtch(charlie.getAddress(), convert(100, tch));
    ...
    public void testRebranch2() throws InterruptedException {
        ...
        bc.submitTransaction(tx1);
        bc.sendtch(alice.getAddress(), BigInteger.valueOf(1000000));
        Block b2 = bc.createBlock();
    ...
    public void testRejected1() throws InterruptedException {
        ...
        for (int i = 0; i < 16; i++) {
            bc.createBlock();
            tcheumListener.PendingTransactionState state = l.pollTxUpdateState(tx1);
            if (state == tcheumListener.PendingTransactionState.DROPPED) {
                break;
            }
            if (i == 15) {
