...
import org.tcheum.core.BlockIdentifier;
...
import org.tcheum.net.eth.message.EthMessage;
import org.tcheum.net.eth.message.EthMessageCodes;
import org.tcheum.net.eth.message.NewBlockHashesMessage;
import org.tcheum.net.eth.message.NewBlockMessage;
import org.tcheum.net.eth.message.StatusMessage;
import org.tcheum.net.eth.message.TransactionsMessage;
...
public class BlockTxForwardTest {
    ...
    static class MinerNode extends BasicSample implements MinerListener{
        ...
        public void run() {
            if (config.isMineFullDataset()) {
                ...
                Ethash ethash = Ethash.getForBlock(config, tcheum.getBlockchain().getBestBlock().getNumber());
                ethash.getFullDataset();
                logger.info("Full dataset generated (loaded).");
            }
            tcheum.getBlockMiner().addListener(this);
            tcheum.getBlockMiner().startMining();
        }
    ...
    static class GeneratorNode extends BasicSample {
        ...
        private void generateTransactions() throws Exception{
            ...
            for (int i = tcheum.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000; i++, j++) {
                {
    ...
    /**
     *  Creating 3 tcheumJ instances with different config classes
    ...
    public void testTest() throws Exception {
        ...
        testLogger.info("Starting tcheumJ miner instance!");
        tcheum miner = tcheumFactory.createtcheum(MinerConfig.class);

        miner.addListener(new tcheumListenerAdapter() {
            @Override
        ...
        tcheumFactory.createtcheum(RegularConfig.class);

        testLogger.info("Starting tcheumJ txSender instance!");
        tcheum txGenerator = tcheumFactory.createtcheum(GeneratorConfig.class);
        txGenerator.addListener(new tcheumListenerAdapter() {
            @Override
            public void onRecvMessage(Channel channel, Message message) {
                super.onRecvMessage(channel, message);
                if (!(message instanceof EthMessage)) return;
