...
import org.tcheum.config.blockchain.DaoNoHFConfig;
import org.tcheum.config.blockchain.FrontierConfig;
import org.tcheum.config.net.BaseNetConfig;
...
//            System.out.println("Created block " + b.getNumber() + " " + getData(b.getExtraData()));
        ...
        assertEquals("tcheumJ powered", getData(sb, FORK_BLOCK - 1));
        assertEquals("dao-hard-fork", getData(sb, FORK_BLOCK));
        assertEquals("dao-hard-fork", getData(sb, FORK_BLOCK + FORK_BLOCK_AFFECTED - 1));
        assertEquals("tcheumJ powered", getData(sb, FORK_BLOCK + FORK_BLOCK_AFFECTED));
    }
